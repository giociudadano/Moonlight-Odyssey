...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommandIconDescription = function (
    command,
    args
  ) {
    switch (getCommandName(command)) {
      case "ADD_ICON_DESC":
      case "アイコン説明追加":
        $gameSystem.addIconDescription(
          getArgNumber(args[0]),
          getArgString(concatArgs(args, 1))
        );
        break;
    }
  };
  ...
  Window_Base.prototype.updateIconTouch = function () {
    ...
    for (var propName in this._iconRects) {
      ...
      if (rectInfo.rect.contains(tx, ty)) {
        var text =
          $gameSystem.getIconDescription(rectInfo.index) ||
          settings.iconDesc[rectInfo.index];
  ...
  Window_Base.prototype.popupCaption = function (text) {
    ...
    if (!this._captionWindow) {
      this._captionWindow = new Window_Caption(
        TouchInput.x,
        TouchInput.y,
        text
      );
  ...
  Window_Base.prototype.isTouchedInsideFrame =
  ...
  //  再描画時にアイコンリストを初期化します。
  //=============================================================================
  var _Window_Selectable_drawAllItems =
  ...
  Window_Caption.prototype.windowWidth = function () {
    return (
      this.drawTextEx(this._text, 0, this.contents.height) + this.padding * 2
    );
  };

  Window_Caption.prototype.windowHeight = function () {
    return (
      this.calcTextHeight({ text: this._text, index: 0 }) + this.padding * 2
    );
  };
